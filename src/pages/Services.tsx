          ...
          {services.map((s, i) => (
            <AnimatedSection key={i}>
              <div className={`grid md:grid-cols-2 gap-12 items-center ${i % 2 === 1 ? "md:flex-row-reverse" : ""}`}>
                ...
                <div className={`relative overflow-hidden rounded-xl ${i % 2 === 1 ? "md:order-1" : ""}`}>
                  <img src={s.image} alt={s.title} className="w-full h-72 object-cover rounded-xl hover:scale-105 transition-transform duration-700" loading="lazy" />
                </div>
              </div>
            </AnimatedSection>
          ))}
