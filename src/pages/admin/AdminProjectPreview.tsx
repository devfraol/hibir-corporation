...
export default function AdminProjectPreview() {
  ...
  return <main className="space-y-6"><Seo title={`${title} | Preview`} description={description} path={`/admin/projects/${project.id}/preview`} image={project.cover_image ?? undefined} noindex/><div className="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-accent/40 bg-accent/10 p-4"><strong className="text-sm tracking-wide">PREVIEW — NOT PUBLISHED</strong><Button asChild variant="outline"><Link to={`/admin/projects/${project.id}/edit`}><ArrowLeft size={16}/>Back to editor</Link></Button></div><article className="overflow-hidden rounded-xl border bg-card">{project.cover_image && <img src={project.cover_image} alt={project.title} className="aspect-[16/7] w-full object-cover"/>}<div className="space-y-6 p-6 md:p-10"><p className="label-eyebrow">{project.category ?? "Project"} · {project.project_status ?? "Status not set"}</p><h1 className="font-display text-4xl font-semibold">{project.title || "Untitled project"}</h1><p className="max-w-3xl whitespace-pre-wrap leading-relaxed text-muted-foreground">{project.description || "No description has been added."}</p><dl className="grid gap-4 sm:grid-cols-2"><div><dt className="text-xs uppercase text-muted-foreground">Location</dt><dd>{project.location || "Not specified"}</dd></div><div><dt className="text-xs uppercase text-muted-foreground">Client</dt><dd>{project.client || "Not specified"}</dd></div></dl></div></article></main>;
}
