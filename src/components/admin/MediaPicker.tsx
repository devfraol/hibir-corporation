...
export function MediaPicker({ open, onOpenChange, onSelect }: MediaPickerProps) {
  ...
  const load = async () => {
    setLoading(true);
    try { setAssets(await getNewsMedia()); } catch { toast.error("Unable to load the media library."); } finally { setLoading(false); }
  };
  ...
  const upload = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    setUploading(true);
    try { await uploadNewsImage("library", file); toast.success("Image uploaded to the library."); await load(); } catch { toast.error("Image upload failed. Please try again."); } finally { setUploading(false); }
  };
  const choose = () => { if (!selected) return; onSelect(selected); onOpenChange(false); setSelected(null); };
  return <Dialog open={open} onOpenChange={onOpenChange}>
    <DialogContent className="max-h-[90vh] max-w-5xl overflow-y-auto">
      <DialogHeader><DialogTitle>Choose from Media Library</DialogTitle><DialogDescription>Select one existing News image, or upload a new one.</DialogDescription></DialogHeader>
